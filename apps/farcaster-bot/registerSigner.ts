...
const OP_PROVIDER_URL = process.env.OP_PROVIDER_URL; // Alchemy or Infura url
const RECOVERY_ADDRESS = (process.env.RECOVERY_ADDRESS || zeroAddress) as Hex; // Optional, using the default value means the account will not be recoverable later if the mnemonic is lost
const SIGNER_PRIVATE_KEY: Hex = (process.env.SIGNER_PRIVATE_KEY ||
  zeroAddress) as Hex; // Optional, using the default means a new signer will be created each time
const HUB_URL = process.env.HUB_URL || 'https://farcaster.xyz';
...
const getOrRegisterFid = async (): Promise<number> => {
  ...
  const fid = Number.parseInt(registerLog.args['id']);
...
const registerFname = async (fid: number) => {
  ...
  const localAccount: LocalAccount<string> = toAccount(account);
  // @ts-ignore
